...
const Hooks_useCallback = () => {
    ...
    // Following getItems function get's re-created having different Reference each time when compoenent re-renders
    ...
    return (
        <>
            <div className='container-fluid my-1 py-1' style={themeStyle}>
                <input className='form-control my-2' type="number" value={number} onChange={(e) => setNumber(parseInt(e.target.value))} />

                <button className='btn btn-outline-secondary' onClick={() => setDark(prevDark => !prevDark)} >Toggle Theme</button>
